...
public class saveStudent {
	...
	public static void main(String[] args) {
		/*
		 * List<PassStudent> list = new ArrayList<PassStudent>(); PassStudent
		 * stu1 = new PassStudent("stu1", "stu1/src/main.java", "Yu-Dong Yao",
		 * 10000000); PassStudent stu2 = new PassStudent("stu2",
		 * "stu2/src/main.java", "Dov Kruger", 10000001); PassStudent stu3 = new
		 * PassStudent("stu3", "stu3/src/main.java", "Hong Man", 10000002);
		 * list.add(stu1); list.add(stu2); list.add(stu3); String root =
		 * "/Users/Steveisno1/Documents/16-17Fall/EE810Java/Final Project/ZIPRULE"
		 * ; try { FileOutputStream fos = new FileOutputStream(root +
		 * "/students.ser"); ObjectOutputStream oos = new
		 * ObjectOutputStream(fos); oos.writeObject(list); oos.close(); } catch
		 * (Exception e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } List<PassStudent> newlist = new
		 * ArrayList<PassStudent>(); try { FileInputStream fis = new
		 * FileInputStream(root + "/students.ser"); ObjectInputStream ois = new
		 * ObjectInputStream(fis); newlist = (ArrayList<PassStudent>)
		 * ois.readObject(); ois.close(); } catch (Exception e) { // TODO
